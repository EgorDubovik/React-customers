				...
				<>
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4">
                     ...
                     <div className='flex items-center justify-center'>
                        <div className='w-20 h-20 rounded-full bg-gray-200 dark:bg-gray-800 flex items-center justify-center text-2xl text-gray-300' style={{ backgroundColor: user.color ?? '#ccc' }}>{getTechAbr(user.name)}</div>
                        <div className='ml-5'>
                     ...
                     <div className='mt-5 flex flex-wrap items-center justify-center gap-3'>
                        {user.roles.map((role:any, index:number) => (
                           <span key={index} className={'badge badge-outline-'+(rolesColor[role.role] ?? 'primary') }>{rolesTitle[role.role] ?? 'unknown'}</span>
                        ))}
                     </div>
						</div>
					</div>
